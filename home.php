...
/* initialize the external events
	...
	var calendar = $('#calendar').fullCalendar($.extend({
		...
		events: {url: "rdv_query.php?action=events&username="+$('#username').val()},
		...
		drop: function(date, allDay) { // this function is called when something is dropped
					...
					"callback": function() {
						...
						if(title != '' && person_id != '' && place_id != ''){
							$.ajax({
								...
								data: 'title='+ title+'&start='+ start +'&end='+ end + '&allDay='+ allDay +'&person='+ person_id +'&place='+ place_id +'&username='+$('#username').val()+'&className='+$extraEventClass,
								type: "POST",
			...
			$.ajax({
				url: 'rdv_query.php?action=getPersonsBySearch&username='+$('#username').val(),
				type: 'get',
				async: false,
			...
			$.ajax({
				url: 'rdv_query.php?action=getPlacesBySearch&username='+$('#username').val(),
				type: 'get',
				async: false,
				success: function(data){
					var opt = $.parseJSON(data);
			...
			$('#txt_person').autocomplete({
				source: "rdv_query.php?action=getPersonsBySearch&username="+$('#username').val(),
				minLength: 2,
			...
			$('#txt_place').autocomplete({
				source: "rdv_query.php?action=getPlacesBySearch",
				minLength: 2,
				select: function(event, ui) {
		...
		select: function(start, end, allDay) {
			...
			//form.append("<div class='control-group'><label class='control-label'>Participant</label><div class='controls'><input type=text name='person' id='txt_person'/><input type='hidden' name='person_id' id='person_id'/></div></div> ");
			//form.append("<div class='control-group'><label class='control-label'>Emplacement</label><div class='controls'><input type=text name='place' id='txt_place'/><input type='hidden' name='place_id' id='place_id' /> </div></div>");
			form.append("<div class='control-group'><label class='control-label'>Participant *</label><div class='controls'><select class='chzn-select' id='select-persons' data-placeholder='Sélectionnez un contact...'></select></div></div>");
			form.append("<div class='control-group'><label class='control-label'>Emplacement *</label><div class='controls'><select class='chzn-select' id='select-places' data-placeholder='Sélectionnez un emplacement...'></select></div></div>");
			$.ajax({
				...
				[
				{
					...
					"callback": function() {
						...
						if(title != '' && person_id != '' && place_id!= ''){
							$.ajax({
								url: 'rdv_query.php?action=addEvent',
								data: 'title='+ title+'&start='+ start +'&end='+ end + '&allDay='+ allDay +'&person='+ person_id +'&place='+ place_id +'&username='+$('#username').val(),
								type: "POST",
								success: function(reponse) {
									if(reponse == 'ok') {
										$.gritter.add({
		...
		eventDrop: function(event, delta) {
			$.ajax({
				url: 'rdv_query.php?action=log',
				data: 'username='+ $('#username').val()+'&object_id='+$("#drop_id").val(),
				type: "POST",
				success: function() {}
		...
		eventResize: function(event) {
			$.ajax({
				url: 'rdv_query.php?action=log',
				data: 'username='+ $('#username').val()+'&object_id='+$("#resize_id").val(),
				type: "POST",
				success: function() {}
		...
		eventClick: function(calEvent, jsEvent, view) {
			...
			/*var form = $("<form class='form-horizontal'></form>");
			form.append("<div class='control-group'><label class='control-label'>Nom de l'évènement</label><div class='controls'><input autocomplete=off type=text value='" + calEvent.title + "' /></div></div> ");
			form.append("<div class='control-group'><label class='control-label'>Participant</label><div class='controls'><input autocomplete=off type=text value='" + calEvent.person_name + " <" + calEvent.label_activity + ">' /></div></div> ");
			form.append("<div class='control-group'><label class='control-label'>Emplacement</label><div class='controls'><input autocomplete=off type=text value='" + calEvent.place_name + " <" + calEvent.place_addr + ">' /> </div></div>");
			form.append("<div class='center'><button type='submit' class='btn btn-small btn-success'><i class='icon-ok'></i> Sauvegarder</button></div>");
			*/
			var form = $("<form class='form-horizontal'></form>");
			...
			//form.append("<div class='control-group'><label class='control-label'>Participant</label><div class='controls'><input type=text  name='person' id='txt_person' value='" + calEvent.person_lastname + " "+ calEvent.person_firstname +" <" + calEvent.activity_label + ">'/><input type='hidden' name='person_id' id='person_id' value='"+calEvent.person_id+"'/></div></div> ");
			//form.append("<div class='control-group'><label class='control-label'>Emplacement</label><div class='controls'><input type=text name='place' id='txt_place' value='" + calEvent.place_name + " <" + calEvent.place_addr + ">'/><input type='hidden' name='place_id' id='place_id' value='"+calEvent.place_id+"'/> </div></div>");
			form.append("<div class='control-group'><label class='control-label'>Participant *</label><div class='controls'><select class='chzn-select' id='select-persons' data-placeholder='Sélectionnez un contact...'></select></div></div>");
			form.append("<div class='control-group'><label class='control-label'>Emplacement *</label><div class='controls'><select class='chzn-select' id='select-places' data-placeholder='Sélectionnez un emplacement...'></select></div></div>");
				...
				[
				{
					...
					"callback": function() {
						...
						if(title != '' && person_id != '' && place_id != ''){
							$.ajax({
								url: 'rdv_query.php?action=updateEvent',
								data: 'id='+calEvent.rdv_id+'&title='+ title+'&start='+ start +'&end='+ end + '&allDay='+ allDay +'&person='+ person_id +'&place='+ place_id +'&username='+$('#username').val(),
								type: "POST",
								success: function(reponse) {
									if(reponse == 'ok') {
			...
			$('#txt_place').autocomplete({
				source: "rdv_query.php?action=getPlacesBySearch&username="+$('#username').val(),
				minLength: 2,
				select: function(event, ui) {
					$('#place_id').val(ui.item.id);
				}
